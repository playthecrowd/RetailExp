 ...
 *
 * Same reason as lib/pilot/access-state.ts: a `"use server"` module may export
 * ONLY async functions, and IDLE_MODERATION_STATE is an object. It would have
 * failed the build identically to the access gate — the moderation dashboard's
 * POSTs, not its GET — so it moves here rather than waiting to be discovered.
 *
